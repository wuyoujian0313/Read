...
- (void)layoutPriceView:(UIView *)viewParent {
    ...
    if (priceLabel == nil) {
        ...
        [priceLabel setTextColor:[UIColor colorWithHex:0x660000]];
    ...
    if (price !=nil && [price length] > 0) {
        ...
        NSDictionary *attributes1 = @{ NSFontAttributeName:[UIFont boldSystemFontOfSize:14], NSForegroundColorAttributeName:[UIColor redColor] };
        
        NSDictionary *attributes2 = @{ NSFontAttributeName:[UIFont systemFontOfSize:11], NSForegroundColorAttributeName:[UIColor redColor] };
...
- (void)favoriteAction:(UIButton *)sender {
    NSString *fav = _bookInfo.isFavor;
    if ([fav isEqualToString:@"yes"]) {
        [sender setImage:[UIImage imageNamed:@"heart_empty"] forState:UIControlStateNormal];
    } else {
        [sender setImage:[UIImage imageNamed:@"heart_read"] forState:UIControlStateNormal];
    }
}
